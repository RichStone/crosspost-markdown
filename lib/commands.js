...
const article = async (articleName, cmdObj) => {
  ...
  if (Object.keys(setupErrors).length) {
    ...
    console.log("You've got to fix these first ☝️")

    if ('devtoApiKeyPresent' in setupErrors) {
      console.log(`
      In case that you do not want to use dev.to, 
      just configure it with a dummy API key for now :)\n
      Run "crosspost configure --only devto" to do it now\n
      and add any piece of text as the key, like "DUMMY".\n
      You can change it later at any time again.
      `)
    }
    return process.exit()
  }
...
module.exports = {
  initializeCommands: async () => {
    ...
    .option('--to <platform>', '(optional) Specify the platform ("webflow" or "devto"). Per default, articles are cross-posted to all platforms.')
    .action(article)
