...
const article = async (articleName, cmdObj) => {
  // FIXME: validate articleName: it should not be a file path, can only be a .md file
  // to ensure that CLI is fired only from the dir of the config
  // FIXME: if the article md doesn't exist exit process ASAP, so that no configs are created etc.
