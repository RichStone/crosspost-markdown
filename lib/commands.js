...
const article = async (articlePath, cmdObj) => {
  console.log(
    chalk.greenBright(
  ...
  configurer.assertCrosspostConf(articlePath)
  ...
  if (targetPlatform) {
    ...
    switch (targetPlatform) {
      case 'webflow':
        webflow.publish(articlePath)
        break
      case 'devto':
        devto.publish(articlePath)
        break
      default:
        console.log(
    ...
    await webflow.publish(articlePath)
    await devto.publish(articlePath)
  }
}
