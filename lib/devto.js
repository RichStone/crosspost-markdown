...
to specify the --canonical-url option. 🚨"
  ...
  publish: async (articleName) => {
    ...
    meta.canonicalUrl = configurer.getCanonicalUrl()
    ...
    let devtoResponse
    const devtoArticleId = await configurer.getCrosspostConfJson('devtoArticleId')
    try {
      if (devtoArticleId) {
    ...
    if (devtoResponse.status < 200 || devtoResponse.status > 299) {
      ...
      configurer.writeToCrosspostConfig('devtoArticleId', devtoResponse.id)
